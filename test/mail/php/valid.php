    ...
    if (isset($_POST['step2']) ) {
        if(empty($_POST['step2']) && STEP1ISREQUIRED) {
            ...
            if (!empty($_POST['step2'])) {
                $step2 = "<b>Тип: </b> " . trim(strip_tags($_POST['step2'])) . "<br>";
            }
        }
    }
    ...
    if (isset($_POST['step3']) ) {
        if(empty($_POST['step3']) && STEP1ISREQUIRED) {
            ...
            if (!empty($_POST['step3'])) {
                $step3 = "<b>Материал столешницы: </b> " . trim(strip_tags($_POST['step3'])) . "<br>";
            }
        }
    }
    ...
    if (isset($_POST['step4']) ) {
        if(empty($_POST['step4']) && STEP1ISREQUIRED) {
            ...
            if (!empty($_POST['step4'])) {
                $step4 = "<b>Площадь кухни: </b> " . trim(strip_tags($_POST['step4'])) . "<br>";
            }
        }
    }
    ...
    if (isset($_POST['step5']) ) {
        if(empty($_POST['step5']) && STEP1ISREQUIRED) {
            ...
            if (!empty($_POST['step5'])) {
                $step5 = "<b>Срок: </b> " . trim(strip_tags($_POST['step5'])) . "<br>";
            }
        }
    }
    ...
    if (isset($_POST['step6']) ) {
        if(empty($_POST['step6']) && STEP1ISREQUIRED) {
            ...
            if (!empty($_POST['step6'])) {
                $step6 = "<b>Бюджет: </b> " . trim(strip_tags($_POST['step6'])) . "<br>";
            }
        }
    }
    if (isset($_POST['step7']) ) {
        if(empty($_POST['step7']) && STEP1ISREQUIRED) {
            $msgs['step7'] = MSGSSTEP1ERROR;
        } else {
            if (!empty($_POST['step7'])) {
                $step7 = "<b>Подсветка: </b> " . trim(strip_tags($_POST['step7'])) . "<br>";
            }
        }
    }
    if (isset($_POST['step8']) ) {
        if(empty($_POST['step8']) && STEP1ISREQUIRED) {
            $msgs['step8'] = MSGSSTEP1ERROR;
        } else {
            if (!empty($_POST['step8'])) {
                $step8 = "<b>Подарок: </b> " . trim(strip_tags($_POST['step8'])) . "<br>";
            }
        }
    }
